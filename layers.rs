...
pub trait Layer<F: Float> {
    ...
    fn flatten(&self, _x: Vec<Vec<F>>, _w: Vec<Vec<F>>, _b: Vec<Vec<F>>) -> (Vec<F>, Vec<F>, Vec<F>){
        todo!("Complete me");
    }

    fn flatten_kernel(&self, _k: Vec<Vec<F>>) -> Vec<F>{
        todo!("Complete me");
    }

    fn backward(&self, _d: (&mut [F], &mut [F], &mut [F]), _l_r: F, _l: F) -> (Vec<F>, Vec<F>) {
